...
public class Booking_ScheduleDAO {
    ...
    public void test() {
        ...
        try {
            // Lấy kết nối từ JDBC
            Connection connection = JDBC.getConnection();
            // Câu lệnh SQL lấy tất cả dữ liệu từ bảng Booking
            ...
            while (rs.next()) {
                ...
                // Tạo đối tượng Status từ cột status_id
                Status status = new Status();
                status.setId(rs.getInt("status_id"));
                booking.setStatus(status);

                // Lấy ngày tạo (create_date) làm Timestamp
                booking.setDate(rs.getTimestamp("create_date"));

                // Tạo đối tượng Mentor với tài khoản được lấy từ cột mentor_id
                Mentor mentor = new Mentor();
                Account mentorAccount = new Account();
                mentorAccount.setId(rs.getString("mentor_id"));
                mentor.setAccount(mentorAccount);
                booking.setMentor(mentor);

                // Tạo đối tượng Mentee với tài khoản được lấy từ cột mentee_id
                Mentee mentee = new Mentee();
                Account menteeAccount = new Account();
                menteeAccount.setId(rs.getString("mentee_id"));
                mentee.setAccount(menteeAccount);
                booking.setMentee(mentee);

                // Lấy ngày bắt đầu và ngày kết thúc
                booking.setStartDate(rs.getDate("from_date"));
                booking.setEndDate(rs.getDate("to_date"));

                // Lấy số lượng slot
                booking.setTotalSlot(rs.getInt("total_slot"));

                // Tạo đối tượng Level_Skills từ cột level_skill_id
                Level_Skills levelSkills = new Level_Skills();
                levelSkills.setId(rs.getInt("level_skill_id"));
                booking.setLevel_skills(levelSkills);

                bookings.add(booking);
            }
            // In ra thông tin từng Booking
            for (Booking b : bookings) {
                System.out.println("Booking ID: " + b.getId() +
                        ", Mentor: " + b.getMentor().getAccount().getId() +
                        ", Mentee: " + b.getMentee().getAccount().getId() +
                        ", Status: " + b.getStatus().getId() +
                        ", Create Date: " + b.getDate() +
                        ", From Date: " + b.getStartDate() +
                        ", To Date: " + b.getEndDate() +
                        ", Total Slot: " + b.getTotalSlot());
            }
